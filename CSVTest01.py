...
def analizesData(html):
    ...
    divs = soup.find('div', class_='hanml').find('div', class_='conMidtab').find_all('div', class_='conMidtab2')

    for div in divs:
        trs = div.select('table tr')[2:]
        province = div.find('td', class_='rowsPan').find('a').string.strip()
        ...
        for tr in trs:
            ...
            weather.weather_day = tds[1].string.strip()  # 日间天气现象
            weather.weather_night = tds[3].string.strip()  # 夜间天气现象
            weather.wind_direction_day = tds[2].find_all('span')[0].string.strip()  # 日间风向
            weather.wind_direction_night = tds[5].find_all('span')[0].string.strip()  # 夜间风向
            weather.wind_power_day = tds[2].find_all('span')[1].string.strip()  # 日间风力等级
            weather.wind_power_night = tds[5].find_all('span')[1].string.strip()  # 夜间风力等级
